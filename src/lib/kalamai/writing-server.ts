...
import {
  ...
  buildSectionDraftPrompt, bandFor,
...
async function stepOutline(db: SupabaseClient, a: ArticleRow): Promise<StepResult> {
  ...
  const meta = buildArticleMeta(brief, plan, a.params.contentType ?? "blog");
...
async function stepScore(db: SupabaseClient, a: ArticleRow): Promise<StepResult> {
  const brief = await loadBrief(db, a);
  // Hard ceiling backstop: never ship over the type's band max even if the model ran long.
  const blocks = enforceWordCap(a.stage_state.blocks ?? [], bandFor(a.params.contentType ?? "blog")[1]);
  const meta = a.stage_state.meta ?? { title: "", description: "", ogTitle: "", ogDescription: "", jsonld: "", contentType: a.params.contentType ?? "blog" };
