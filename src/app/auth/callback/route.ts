 ...
 * to the return path or identity default. Any failure drops back to /login
 * with a flag the page surfaces.
 */
export async function GET(request: NextRequest) {
  ...
  const dest = loginDestination(searchParams.get("next"), data.user?.email);
