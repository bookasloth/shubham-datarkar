...
import { getProfileByUsername, listFollowers } from "@/lib/community/queries";
...
export async function generateMetadata({
  ...
  return buildMetadata({
    ...
    path: `/community/u/${username}/followers`,
    noIndex: true,
  });
}
...
export default async function FollowersPage({
  ...
  if (!user)
    redirect(`/members/login?next=/community/u/${profile.username}/followers`);

  const people = await listFollowers(profile.id);
  ...
  return (
    <div>
      ...
      <PeopleList people={people} empty="Nobody follows this account yet." />
    </div>
  );
}
