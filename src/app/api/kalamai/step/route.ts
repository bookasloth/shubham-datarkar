...
export const maxDuration = 60; // Hobby ceiling; covers a 6-URL crawl batch or the R4 call

/** Advance one analysis job by exactly one transition. Ownership-checked. */
export async function POST(req: NextRequest) {
  ...
  const { data } = await supabaseAdmin().from("kalamai_analyses").select("user_id").eq("id", id).maybeSingle();
