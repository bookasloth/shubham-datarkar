...
// No dedicated shelves-index route in this slice — the shelves rail lives on
// /books itself, so we skip a "Collections" tab rather than link a 404.
const LINKS = [
  { href: "/books", label: "Discover" },
  { href: "/books/my-list", label: "My List" },
];

export function BookNav() {
  const pathname = usePathname();
  const isActive = (href: string) =>
    href === "/books" ? pathname === "/books" : pathname.startsWith(href);

  return (
    <div className="mb-6 flex flex-wrap items-center justify-between gap-3">
      <nav aria-label="Books" className="flex items-center gap-1">
        {LINKS.map((l) => (
          <Link
            key={l.href}
            href={l.href}
      ...
      <form action="/books/search" className="relative">
