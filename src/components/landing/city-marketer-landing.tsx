...
import type { CityMarketerContent } from "@/lib/data/landing/digital-marketer-cities";
...
/**
 * Shared body for every /digital-marketer-in-<location> page. The JSX is
 * identical across cities; only the `c` content object and the areaServed in
 * the ProfessionalService node vary. The Person address stays Nagpur (real) —
 * areaServed conveys the target market, so Pune/Mumbai/India pages are honest
 * about where Shubham is based.
 */
export function CityMarketerLanding({ c }: { c: CityMarketerContent }) {
  const localBusinessSchema = {
    "@context": "https://schema.org",
    "@type": "ProfessionalService",
    "@id": `${site.url}${c.path}#business`,
    name: `Digital Marketer in ${c.location} — Shubham Datarkar`,
    ...
    <>
      <JsonLd
        data={[
          ...
          breadcrumbSchema([
            { name: "Home", path: "/" },
            { name: `Digital Marketer in ${c.location}`, path: c.path },
          ]),
          faqSchema(c.faqs),
        ]}
      />
      ...
      <PageHero
        ...
        actions={
          <div className="flex w-full max-w-xs flex-col items-stretch gap-3">
            ...
            <Link href="#case-studies" className={cn(buttonVariants({ variant: "outline", size: "lg" }), "w-full")}>
              See results
              <ArrowRight />
            </Link>
          </div>
      ...
      <Section className="py-16 md:py-20">
        <Container>
          <h2 className="text-sm font-semibold uppercase tracking-[0.12em] text-muted-foreground">Why businesses hire me</h2>
      ...
      <Section bleed className="border-y border-border bg-muted py-16 md:py-20">
        <Container>
          <h2 className="text-sm font-semibold uppercase tracking-[0.12em] text-muted-foreground">
            Is your marketing facing these problems?
          </h2>
      ...
      <Section bleed className="border-y border-border bg-muted py-16 md:py-20">
        <Container>
          <h2 className="text-sm font-semibold uppercase tracking-[0.12em] text-muted-foreground">My marketing process</h2>
      ...
      <Section bleed className="border-y border-border bg-muted py-16 md:py-20">
        <Container>
          <h2 className="text-sm font-semibold uppercase tracking-[0.12em] text-muted-foreground">Industries I work with</h2>
      ...
      <Section id="case-studies" className="scroll-mt-20 py-16 md:py-20">
        <Container>
          <h2 className="text-sm font-semibold uppercase tracking-[0.12em] text-muted-foreground">Client success stories</h2>
          ...
          <Accordion type="single" collapsible className="mt-4">
            {c.faqs.map((f) => (
              <AccordionItem key={f.question} value={f.question}>
                ...
                {/* forceMount: keep answers in the server HTML for AI extraction. */}
        ...
        <Container>
          <Reveal>
            <div className="mx-auto flex max-w-3xl flex-col items-center gap-6 text-center">
              ...
              <div>
                <h2 className="text-sm font-semibold uppercase tracking-[0.12em] text-muted-foreground">{c.about.heading}</h2>
