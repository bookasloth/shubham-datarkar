...
--
-- Reblogs: a reblog is a community_posts row with reblog_of set and no body.
-- The feed RESOLVES it to the source post, so the card shows the original's
-- content/author/counts attributed as "@reblogger reblogged". Without this the
-- feed renders bodyless ghost cards.
--   row_id  = the feed row (reblog row or the post itself) — use as React key.
--   id      = the SOURCE post — votes/replies/bookmarks all target it.
-- =====================================================================

-- Old versions must go, or the new arg lists become ambiguous overloads.
...
language sql stable security definer set search_path = public as $$
  ...
  filtered as (
    select * from src
    -- Author + bookmark views are personal: never hide the viewer's own
    -- controversial or moderator-demoted posts from them.
    where (p_author is not null or p_bookmarked
           or (not demoted
...
language sql stable security definer set search_path = public as $$
  select p.id, p.id, p.user_id, pr.username, pr.display_name,
